...
class AddFavoriteExchangeUseCase @Inject constructor(
    private val repository: FavoriteRatesDatabaseRepository
) {

    suspend operator fun invoke(exchangeRateDomain: ExchangeRateDomain) {
        return repository.addFavoriteExchange(exchangeRateDomain)
    }
}